-- Unlock tiers: one coherent ladder (DB now matches src/lib/unlock-tiers.ts).
--
-- Before this migration the frontend showed a 17-step ladder starting at $20
-- while the DB awarded 9 badges starting at $2,000 — and the label function
-- was shifted so e.g. enum 'all_about_the_benjamin' displayed as "Quiet
-- Backer". Now: 9 tiers, enum names match their labels, attainable entry
-- tier, and the Benjamin pair front and center.

create or replace function public.unlock_tier_for_gross(gross_cents bigint)
returns public.unlock_tier
language sql
immutable
as $$
  select case
    when gross_cents >= 10000000 then 'legend'::public.unlock_tier             -- $100,000+
    when gross_cents >= 5000000  then 'whale'::public.unlock_tier              -- $50,000+
    when gross_cents >= 1000000  then 'midas'::public.unlock_tier              -- $10,000+
    when gross_cents >= 500000   then 'cash_vault'::public.unlock_tier         -- $5,000+
    when gross_cents >= 100000   then 'money_printer'::public.unlock_tier      -- $1,000+
    when gross_cents >= 50000    then 'stacking_hundreds'::public.unlock_tier  -- $500+
    when gross_cents >= 20000    then 'all_about_the_benjamins'::public.unlock_tier -- $200+
    when gross_cents >= 10000    then 'all_about_the_benjamin'::public.unlock_tier  -- $100+
    when gross_cents >= 2000     then 'first_bill'::public.unlock_tier         -- $20+
    else 'none'::public.unlock_tier
  end;
$$;

create or replace function public.unlock_tier_label(tier public.unlock_tier)
returns text
language sql
immutable
as $$
  select case tier
    when 'first_bill' then 'First Bill'
    when 'all_about_the_benjamin' then 'It''s All About the Benjamin'
    when 'all_about_the_benjamins' then 'It''s All About the Benjamins'
    when 'stacking_hundreds' then 'Stacking Hundreds'
    when 'money_printer' then 'Money Printer'
    when 'cash_vault' then 'Cash Vault'
    when 'midas' then 'Midas Touch'
    when 'whale' then 'The Whale'
    when 'legend' then 'Unmaskr Legend'
    else null
  end;
$$;

create or replace function public.unlock_tier_icon(tier public.unlock_tier)
returns text
language sql
immutable
as $$
  select case tier
    when 'first_bill' then '🧾'
    when 'all_about_the_benjamin' then '💵'
    when 'all_about_the_benjamins' then '💵💵'
    when 'stacking_hundreds' then '💰'
    when 'money_printer' then '🖨️💸'
    when 'cash_vault' then '🏦'
    when 'midas' then '👑✨'
    when 'whale' then '🐋'
    when 'legend' then '👁️⚡'
    else null
  end;
$$;

-- Recompute tiers for all profiles under the new thresholds
update public.profiles
set unlock_tier = public.unlock_tier_for_gross(unlock_gross_cents);
