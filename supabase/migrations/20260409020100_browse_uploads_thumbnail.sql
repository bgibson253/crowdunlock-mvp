-- Update browse_uploads RPC to include thumbnail_url and file_path for image previews
-- Must drop first because return type changed (can't use create or replace)

drop function if exists public.browse_uploads(text, text, text, integer, integer, uuid);
...
-- Drop old signatures handled above
-- DO $$ blocks removed since we dropped explicitly above
